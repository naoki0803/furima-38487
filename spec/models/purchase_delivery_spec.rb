  ...
  describe '購入機能' do
    ...
    context '内容に問題がある場合' do
      it 'token空だと保存できないこと' do
        ...
        expect(@purchase_delivery.errors.full_messages).to include("クレジットカード情報を入力してください")
      end
      ...
      it 'user_idが紐付いていないと保存できないこと' do
        ...
        @purchase_delivery.valid?
        expect(@purchase_delivery.errors.full_messages).to include("ユーザーを入力してください")
      end
      it 'item_idが紐付いていないと保存できないこと' do
        @purchase_delivery.item_id = nil
        @purchase_delivery.valid?
        expect(@purchase_delivery.errors.full_messages).to include("商品を入力してください")
      end
    end
  end
end
